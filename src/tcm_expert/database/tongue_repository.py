        ...
        with self.database.transaction() as connection:
            ...
            cursor = connection.execute(
                """INSERT INTO tongue_analyses
                   ...
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (consultation_id, required_text(image_path, "Ảnh", 2000),
                 result["image_sha256"], result["image_width"], result["image_height"],
                 result["quality_score"], "\n".join(result["quality_issues"]),
                 result["segmentation_confidence"], result["tongue_color"],
                 result["coating_color"], result["coating_thickness"],
                 int(result["teeth_marks"]), int(result["cracks"]),
                 result["ai_confidence"], json.dumps(result["metrics"], ensure_ascii=False)),
            )
        ...
        with self.database.transaction() as connection:
            ...
            connection.execute(
                ...
                (optional_text(values.get("tongue_color"), 100),
                 ...
                 int(bool(values.get("teeth_marks"))), int(bool(values.get("cracks"))),
                 optional_text(values.get("note"), 3000), reviewer, analysis_id),
            )
